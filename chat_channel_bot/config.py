...
class Config:
    ...
    @staticmethod
    def init_config(file, prefix=None):
        if file and os.path.exists(file):
            Config.config.read(os.path.abspath(file))
        if prefix:
    ...
    def get_or_else(section, option, default_value):
        if Config.config.has_option(section, option):
            ...
            return os.environ.get("_".join([Config.prefix, section.upper(), option]), default_value)
    ...
    def _get_conv_env_or_else(section, option, conv, default_value):
        return conv(os.environ.get("_".join([Config.prefix, section.upper(), option.upper()]), default_value))
